...
// Allowlist of valid settings keys derived from SettingsSchema
...
export function setupSettingsHandlers(): void {
  ...
  ipcMain.handle('settings:set', (_event, key: keyof SettingsSchema, value: any) => {
    try {
      if (!ALLOWED_SETTINGS_KEYS.has(key as string)) {
  ...
  ipcMain.handle(
    ...
    async (_event, enabled: boolean, startMinimized: boolean) => {
      try {
