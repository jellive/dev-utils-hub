...
function rotateBackups(): void {
  ...
  try {
    ...
    if (files.length > config.maxBackups) {
      ...
      console.log(
        `✓ Backup rotation complete: kept ${config.maxBackups}, deleted ${filesToDelete.length}`
      );
    }
  } catch (error) {
...
export function restoreFromBackup(backupPath: string): void {
  ...
  if (!resolvedPath.startsWith(backupsDir + sep)) {
    throw new Error('Invalid backup path: must be within backups directory');
  }
  ...
  try {
    ...
    console.log(`✓ Restored database from: ${resolvedPath}`);
